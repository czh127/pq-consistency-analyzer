...
CONFIG = {
    ...
    # ====================== 图例独立开关 1=显示 0=隐藏 ======================
    "PERF_SHOW_LEGEND": 0,                # 单文件性能图
    "PQ_COMPARE_SHOW_LEGEND": 1,          # 全部产品PQ对比总图
    "HYSTERESIS_SHOW_LEGEND": 1,          # 迟滞图
    "CON_SHOW_LEGEND": 0,                 # 一致性图
    "AVG_PRESS_SHOW_LEGEND": 0,           # 压差平均值图
    "GOBACK_SHOW_LEGEND":0,               # 单文件去程/回程/合并PQ图
    "GOBACK_SUMMARY_SHOW_LEGEND":0,       # 全体去回程总对比大图
    ...
    # ====================== 去程回程PQ图表独立配置（新增，不和PERF共用） ======================
    "GOBACK_FIG_SIZE": [12, 6],           # 单文件去程回程画布尺寸
    "GOBACK_DPI": 100,                    # 单文件PQ图片DPI
    "GOBACK_LINE_COLOR": "#00B0F0",       # 去程回程曲线颜色
...
if CONFIG["RUN_PQ_COMPARE_PLOT"] and pq_comparison_data:
    ...
    if CONFIG["PQ_COMPARE_SHOW_LEGEND"] == 1:
...
if CONFIG["RUN_HYSTERESIS_PLOT"]:
    ...
    if CONFIG["HYSTERESIS_SHOW_LEGEND"] == 1:
...
if CONFIG["RUN_AVG_PRESSURE_PLOT"]:
    ...
    if CONFIG["AVG_PRESS_SHOW_LEGEND"] == 1:
...
if CONFIG["RUN_SINGLE_GOBACK_PQ"]:
    ...
    for file_idx, file in enumerate(csv_files, 1):
        ...
        if CONFIG["GOBACK_SHOW_LEGEND"] == 1:
...
if CONFIG["RUN_ALL_GOBACK_SUMMARY"] and CONFIG["RUN_SINGLE_GOBACK_PQ"]:
    ...
    if CONFIG["GOBACK_SUMMARY_SHOW_LEGEND"] == 1:
